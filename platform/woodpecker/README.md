...
| `kv/data/woodpecker/grpc-secret` | `token` | Server gRPC JWT signing secret (`WOODPECKER_GRPC_SECRET`). Random, script-seeded: `seed-random-secret.sh kv/woodpecker/grpc-secret token`. Pinned 2026-09-13 — unset, the server re-randomises it per start and the agent must re-register. Server-only; rotation = re-seed `--force`, refresh the ExternalSecret, delete the server pod (OnDelete) in a no-CI window — the agent reconnects with its token. |
...
| `kv/data/argocd/registry-pull` | `username`, `token` | **Not seeded here** — minted for Argo's repo-server (`bootstrap/argocd/`, `provision-forgejo-bot-pat.sh --bot-username argocd-chart-pull --scopes read:package`). Projected into `ci-woodpecker` as the dockerconfigjson `registry-homelab-internal-pull` (`registry-pull-secret.yaml`), which the agent attaches to every step Pod via `WOODPECKER_BACKEND_K8S_PULL_SECRET_NAMES` so the chart workflows can pull the first-party ci-tools image (ADR 0068). `read:package` only, on purpose: it is on every step Pod, pull requests included. |
