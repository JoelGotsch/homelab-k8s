...
| `kv/data/conversation-history-v2/oidc` | `client_id`, `client_secret`, `owner_username` | Dedicated archive owner login; `conversation-history-v2/scripts/homelab_identity.py` validates the selected active owner and blueprint before create-only seeding. Only that owner is admitted, with a strict callback and authorization-code grant. The app separately verifies issuer/subject against its existing owner principal. |
