...
| `check-retention-render-fixtures.sh` | offline input/package provenance and an exact projection cover all chart-generated Longhorn claims in Langfuse, CrowdSec, and Woodpecker, all rendered Langfuse S3 `secretKeyRef` name/key pairs, and each target Secret/key → OpenBao key/property tuple in both ExternalSecret source mirrors; `--render` verifies a freshly downloaded pinned chart and rejects any generated claim with an absent/empty class, without contacting Kubernetes | pre-commit (offline), manual after chart bumps (`--render`) |
