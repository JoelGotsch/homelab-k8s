...
| [`check-retention-contract.sh`](scripts/check-retention-contract.sh) | every cluster-owned Longhorn selector is classified in the finite [lifecycle contract](scripts/retention-contract.md), exact source/mirror values match, and existing Delete/Retain mismatches cannot expand silently; app-owned repositories carry their own contract | pre-commit |
| `check-retention-render-fixtures.sh` | offline input/package provenance and an exact projection cover all chart-generated Longhorn claims in Langfuse, CrowdSec, and Woodpecker, all rendered Langfuse S3 `secretKeyRef` name/key pairs, and each target Secret/key → OpenBao key/property tuple in both ExternalSecret source mirrors; `--render` verifies a freshly downloaded pinned chart and rejects absent or present-empty classes except the exact bounded Woodpecker-agent omitted-key exception, while proving its effective cluster default remains `longhorn-replica2`, without contacting Kubernetes | pre-commit (offline), manual after chart bumps (`--render`) |
| `check-langfuse-minio-ingress.sh` | MinIO TCP/9000 ingress is limited to Langfuse web/worker pods using the exact component labels proven by the render projection | pre-commit |
