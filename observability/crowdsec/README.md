...
| `kv/data/crowdsec/lapi-auth` | `cs_lapi_secret`, `registration_token` | LAPI shared secret (>64 chars) + agent auto-registration token (>48 chars). Random, script-seeded: `seed-random-secret.sh --size 48 kv/crowdsec/lapi-auth cs_lapi_secret` and `--size 32 … registration_token`. Pinned 2026-08-16 so the chart stops minting them per render; rotation = re-seed with `--force`, refresh the ExternalSecret, restart LAPI (agent re-registers on its next restart). |
