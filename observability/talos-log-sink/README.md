...
- Writes them to a Longhorn `longhorn-replica2` PVC mounted at
  `/sink`, one file per day (`talos-YYYY-MM-DD.log`).
...
No auth. No TLS. No OpenBao / ESO / Loki dependency. Closed
loop inside the cluster, survives a reboot of any single node
because the PVC is Longhorn-replicated.
...
| `pvc.yaml` | Longhorn-replica2, 20 Gi |
...
- Log volume on a 6-node cluster is small (~MB/day per node
  steady-state); 20 Gi PVC at replica-2 gives months of
  retention without rotation. We don't rotate automatically;
  operator runs `kubectl exec ... -- rm /sink/talos-OLD.log`
  during quarterly housekeeping.
